/*
  # Create RPC Function: update_company_and_address (v5 - Use %L Consistently)
  ...
  **Version 5:** Attempts to fix a persistent syntax error (`mismatched parentheses` at line 72)
  by consistently using the `%L` (literal) format specifier for *all* value types within the
  dynamic SQL generation. This ensures proper quoting for all values interpolated into the
  `UPDATE` statements.
  ...
  1.  **Function:**
      ...
      - **Logic:**
          ...
          - Dynamically builds and executes UPDATE statement for `public.companies` based on allowed keys in `p_company_updates`. Uses `%L` for all values. Automatically sets `updated_at`.
          - Retrieves the company's current `address_id`.
          - If `address_id` exists and `p_address_updates` is provided: Dynamically builds and executes UPDATE statement for `public.addresses` based on allowed keys. Uses `%L` for all values. Automatically sets `updated_at` via trigger.
          - If `address_id` does *not* exist and `p_address_updates` is provided: Dynamically builds and executes INSERT statement for `public.addresses`, setting `company_id`. Sets default `location_type` to 'company' if not provided. Updates `public.companies.address_id` with the new address ID.
        ...
        FOR v_key IN SELECT k FROM jsonb_object_keys(p_company_updates) k LOOP
            IF v_key = ANY(v_allowed_company_keys) THEN
                ...
                ELSIF jsonb_typeof(v_value) = 'string' THEN
                   -- FIX v5: Use %L for string literals (value already extracted as text)
                   v_update_fragment := format('%I = %L', v_key, v_value #>> '{}');
                ELSE -- Numbers, booleans
                   -- FIX v5: Use %L for non-string literals after casting to text
    ...
    IF p_address_updates IS NOT NULL AND jsonb_object_keys(p_address_updates)::text[] <> ARRAY[]::text[] THEN
            ...
            FOR v_key IN SELECT k FROM jsonb_object_keys(p_address_updates) k LOOP
                 IF v_key = ANY(v_allowed_address_keys) THEN
                    ...
                    IF v_value IS NULL OR jsonb_typeof(v_value) = 'null' THEN
                        v_update_fragment := format('%I = NULL', v_key);
                    ELSIF jsonb_typeof(v_value) = 'string' THEN
                       -- FIX v5: Use %L for string literals
                       v_update_fragment := format('%I = %L', v_key, v_value #>> '{}');
                    ELSE -- Numbers, booleans
                       -- FIX v5: Use %L for non-string literals after casting to text
                       v_update_fragment := format('%I = %L', v_key, v_value::text);
        ...
        ELSE
            ...
            BEGIN
                FOR v_key IN SELECT k FROM jsonb_object_keys(p_address_updates) k LOOP
                     IF v_key = ANY(v_allowed_address_keys) THEN
                        ...
                        v_insert_cols := array_append(v_insert_cols, format('%I', v_key));
                         ...
                         ELSIF jsonb_typeof(v_value) = 'string' THEN
                            -- Use %L for literal value in INSERT
                            v_insert_vals := array_append(v_insert_vals, format('%L', v_value #>> '{}'));
                         ELSE -- Numbers, booleans
                            -- Use %L for literal value in INSERT after casting
                ...
                IF array_length(v_insert_cols, 1) > 1 THEN
                    ...
                    UPDATE public.companies SET address_id = v_new_address_id WHERE id = v_company_id;
